package br.com.dvlima.ocpjp8.apress.topic05._java_stream_api;
...
// • You can match for a given predicate in a stream using the allMatch(), noneMatch(), and anyMatch() methods.
// Unlike the anyMatch() method that returns false when the stream is empty, the allMatch() and noneMatch()
// methods return true if the stream is empty.
// • You can look for elements in a stream using the findFirst() and findAny() methods.
//      The findAny() method is faster to use than the findFirst() method in case of parallel streams.
...
// • When there are no entries in a stream and operations such as max are called, then instead of returning null
// or throwing an exception, the (better) approach taken in Java 8 is to return Optional values.
// • Primitive type versions of Optional<T> for int, long, and double are OptionalInteger, OptionalLong, and OptionalDouble.
...
// have methods for calculating count, sum, average, minimum, and maximum values of elements in the stream.


//  Sort a collection using Stream API
// • One way to sort a collection is to get a stream from the collection and call sorted() method on that stream.
// The sorted() method sorts the elements in the stream in natural order
// (it requires that the stream elements implements the Comparable interface).
...
// • You can separate the elements in a stream based on a condition (given as a Predicate) using the partition()
